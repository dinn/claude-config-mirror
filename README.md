...
2. 원격 repo visibility가 PUBLIC — 클라우드 routine이 PAT 없이 raw fetch할 수 있도록 의식적 PUBLIC. 실수로 PRIVATE로 바뀌면 push 거부 (routine 깨짐 방지).

WL에 등록된 헤더가 source에 없어도 거부하지 않음. 그 섹션이 미러에 포함되지 않을 뿐.

미러가 PUBLIC이라는 사실 자체가 WL을 엄격히 운영해야 한다는 뜻이다. 사내·민감 내용을 절대 WL에 넣지 말 것.
